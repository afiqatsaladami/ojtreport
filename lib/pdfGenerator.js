...
export function generateWeekPdf(config, weekDays, lampiran) {
  ...
  weekDays.forEach((day, dayIndex) => {
    ...
    // Teks aslinya disimpan terpisah supaya bisa digambar manual (justify
    // pintar) lewat hook didDrawCell, bukan lewat rendering default autoTable.
    ...
    const body = activities.map((act, i) => {
      ...
      const row = [
        ...
        { content: "", styles: { ...cellStyle } }, // Kegiatan digambar manual
        { content: "", styles: { ...cellStyle } }, // Critical Point digambar manual
      ];
      if (i === 0) {
        row.push({
          content: "",
