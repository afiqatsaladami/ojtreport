...
// border tegas di setiap sel. Tinggi baris dihitung dari layoutUtils supaya
// tabel selalu memenuhi 1 halaman, dan teks dipotong otomatis (jaring
// pengaman terakhir) supaya isi tabel tidak pernah meluber ke halaman baru.
const BLACK = [0, 0, 0];
...
export function generateWeekPdf(config, weekDays) {
  ...
  weekDays.forEach((day, dayIndex) => {
    ...
    const body = activities.map((act, i) => {
      // Jaring pengaman terakhir: potong teks kalau melebihi batas muat,
      // supaya tabel dijamin tidak pernah meluber ke halaman baru.
      const kegiatan = truncateToLimit(act.kegiatan || "", kegiatanMax);
      const criticalPoint = truncateToLimit(act.criticalPoint || "", criticalPointMax);

      const row = [String(i + 1), kegiatan, criticalPoint];
      if (i === 0) {
        row.push({
          content: "",
          rowSpan: numRows,
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        1: { cellWidth: COL_WIDTHS_MM.kegiatan, halign: "justify", valign: "middle" },
        // Critical Point Pembelajaran: rata kiri-kanan (justify), vertikal di tengah
        2: { cellWidth: COL_WIDTHS_MM.criticalPoint, halign: "justify", valign: "middle" },
